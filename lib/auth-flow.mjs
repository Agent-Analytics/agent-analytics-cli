...
export async function startDetachedLogin(api, {
  clientType = 'cli',
  clientName = 'Agent Analytics CLI',
} = {}) {
...
export async function loginDetached(api, {
  ...
  const started = await startDetachedLogin(api, { clientType, clientName });
  ...
  while (Date.now() < deadline) {
    ...
    if (status.status === 'exchanged') {
      const error = new Error('auth request already exchanged');
      error.code = 'AUTH_REQUEST_ALREADY_EXCHANGED';
      error.authRequestId = started.auth_request_id;
      throw error;
    }
