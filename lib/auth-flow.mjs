    ...
    server.listen(0, '127.0.0.1', async () => {
      try {
        ...
        const started = await api.startAgentSession({
          ...
          scopes: ['account:read', 'projects:write', 'analytics:read', 'experiments:write', 'feedback:write', 'live:read'],
...
export async function loginDetached(api, {
  ...
  throw new Error(`Timed out waiting for detached approval after ${formatDurationMs(timeoutMs)}`);
}
