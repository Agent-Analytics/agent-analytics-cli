...
export class AgentAnalyticsAPI {
  ...
  async getEvents(project, { event, days = 7, since, limit = 100, user_id, email_hash } = {}) {
    const resolvedSince = since || `${days}d`;
    return this.request('GET', `/events?${this._qs({ project, since: resolvedSince, limit, event, user_id, email_hash })}`);
  }

  async getJourney(project, { days = 30, since, limit = 100, user_id, email_hash } = {}) {
    const resolvedSince = since || `${days}d`;
    return this.request('GET', `/journey?${this._qs({ project, since: resolvedSince, limit, user_id, email_hash })}`);
  }
  ...
  async query(project, { metrics, group_by, filters, date_from, date_to, order_by, order, limit, count_mode, email_hash } = {}) {
    return this.request('POST', '/query', { project, metrics, group_by, filters, date_from, date_to, order_by, order, limit, count_mode, email_hash });
  }
