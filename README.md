...
```bash
npx --yes @agent-analytics/cli@0.5.27 demo
npx --yes @agent-analytics/cli@0.5.27 --demo projects
npx --yes @agent-analytics/cli@0.5.27 --demo funnel agentanalytics-demo --steps "page_view,signup_started,signup"
npx --yes @agent-analytics/cli@0.5.27 --demo experiments list agentanalytics-demo
```
...
npx --yes @agent-analytics/cli@0.5.27 scan https://mysite.com --json

# 2. Sign in when you want the full instrumentation plan
npx --yes @agent-analytics/cli@0.5.27 login

# 3. Create or identify the matching project and primary surface URL
npx --yes @agent-analytics/cli@0.5.27 create my-site --domain https://mysite.com

# 4. Run the full signed-in analysis for that project's surface
npx --yes @agent-analytics/cli@0.5.27 scan https://mysite.com --full --project my-site --json

# Or resume and upgrade the anonymous analysis after login
npx --yes @agent-analytics/cli@0.5.27 scan \
  --resume <analysis_id> \
...
# 3. Watch it live
npx --yes @agent-analytics/cli@0.5.27 live

# Optional detached login for remote or issue-based agent work
npx --yes @agent-analytics/cli@0.5.27 login --detached

# Optional: clear your saved local auth later
npx --yes @agent-analytics/cli@0.5.27 logout
```
...
```bash
npx --yes @agent-analytics/cli@0.5.27 upgrade-link --detached \
  --reason "Need funnel and retention reads for this analysis" \
  --command "npx --yes @agent-analytics/cli@0.5.27 funnel my-site --steps page_view,signup,purchase"
```
...
```bash
npx --yes @agent-analytics/cli@0.5.27 update stylio --origins 'https://stylio.app,http://lvh.me:3101'
```
...
```bash
npx --yes @agent-analytics/cli@0.5.27 query my-site --filter '[{"field":"properties.referrer","op":"contains","value":"clawflows.com"}]'
```
...
```bash
npx --yes @agent-analytics/cli@0.5.27 properties my-site
npx --yes @agent-analytics/cli@0.5.27 properties-received my-site
npx --yes @agent-analytics/cli@0.5.27 context set my-site --json '{
...
```bash
npx --yes @agent-analytics/cli@0.5.27 feedback \
...
npx --yes @agent-analytics/cli@0.5.27 login --detached
npx --yes @agent-analytics/cli@0.5.27 auth status
```
...
For a local shell where it is useful to keep waiting, use `npx --yes @agent-analytics/cli@0.5.27 login --detached --wait`.
...
```bash
npx --yes @agent-analytics/cli@0.5.27 projects
```
