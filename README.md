...
```bash
npx --yes @agent-analytics/cli@0.5.31 demo
npx --yes @agent-analytics/cli@0.5.31 --demo projects
npx --yes @agent-analytics/cli@0.5.31 --demo funnel agentanalytics-demo --steps "page_view,signup_started,signup"
npx --yes @agent-analytics/cli@0.5.31 --demo experiments list agentanalytics-demo
```
...
npx --yes @agent-analytics/cli@0.5.31 scan https://mysite.com --project my-site --json

# 4. Run the full signed-in analysis when you need the deeper plan
npx --yes @agent-analytics/cli@0.5.31 scan https://mysite.com --full --project my-site --json

# Or resume and upgrade a browser preview after login
npx --yes @agent-analytics/cli@0.5.31 scan \
  --resume <analysis_id> \
...
# 3. Watch it live
npx --yes @agent-analytics/cli@0.5.31 live
...
```bash
npx --yes @agent-analytics/cli@0.5.31 upgrade-link --detached \
  --reason "Need funnel and retention reads for this analysis" \
  --command "npx --yes @agent-analytics/cli@0.5.31 funnel my-site --steps page_view,signup,purchase"
```
...
```bash
npx --yes @agent-analytics/cli@0.5.31 update stylio --origins 'https://stylio.app,http://lvh.me:3101'
```
...
```bash
npx --yes @agent-analytics/cli@0.5.31 query my-site --metrics event_count --count-mode raw
```
...
npx --yes @agent-analytics/cli@0.5.31 funnel shop --steps-json ./funnel.json --json
```
...
```bash
npx --yes @agent-analytics/cli@0.5.31 query my-site --filter '[{"field":"properties.referrer","op":"contains","value":"clawflows.com"}]'
```
...
npx --yes @agent-analytics/cli@0.5.31 properties-received my-site
npx --yes @agent-analytics/cli@0.5.31 context set my-site --json '{
...
```bash
npx --yes @agent-analytics/cli@0.5.31 feedback \
...
For a local shell where it is useful to keep waiting, use `npx --yes @agent-analytics/cli@0.5.31 login --detached --wait`.
