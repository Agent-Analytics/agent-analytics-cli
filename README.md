...
# 3. Watch it live
npx --yes @agent-analytics/cli@0.5.28 live
...
npx --yes @agent-analytics/cli@0.5.28 logout
```
...
```bash
npx --yes @agent-analytics/cli@0.5.28 upgrade-link --detached \
  --reason "Need funnel and retention reads for this analysis" \
  --command "npx --yes @agent-analytics/cli@0.5.28 funnel my-site --steps page_view,signup,purchase"
```
...
```bash
npx --yes @agent-analytics/cli@0.5.28 update stylio --origins 'https://stylio.app,http://lvh.me:3101'
```
...
```bash
npx --yes @agent-analytics/cli@0.5.28 query my-site --metrics event_count --count-mode raw
```
...
npx --yes @agent-analytics/cli@0.5.28 funnel shop --steps-json ./funnel.json --json
```
...
```bash
npx --yes @agent-analytics/cli@0.5.28 query my-site --filter '[{"field":"properties.referrer","op":"contains","value":"clawflows.com"}]'
```
...
npx --yes @agent-analytics/cli@0.5.28 properties-received my-site
npx --yes @agent-analytics/cli@0.5.28 context set my-site --json '{
...
```bash
npx --yes @agent-analytics/cli@0.5.28 feedback \
...
For a local shell where it is useful to keep waiting, use `npx --yes @agent-analytics/cli@0.5.28 login --detached --wait`.
